...
plot_gene_loadings <- function(index, max_loading = F, flip = F) {
  gene_df <- get_gene_mat(webster_depmap)[,index] %>%
    ...
    dplyr::mutate(symbol = convert_cds_to_symbol(CDS_ID))
  ...
  gene_df <- gene_df %>%
    dplyr::mutate(Loadings = Loadings * direction_indicator,
...
plot_cell_loadings <- function(index, max_loading = F, annot = NA) {
  ...
  cell_df <- get_cell_mat(webster_depmap)[,index] %>%
    ...
    arrange(Loadings) %>%
    dplyr::mutate(Name = convert_depmap_id_to_ccle(DepMap_ID),
...
factor_gost_results %>%
  ...
  dplyr::select(Name, Frac_Essential, Loaded_Genes,everything()) %>%
...
factor_pleiotrop_df %>% select(Function.x, Function.y) %>%
  count(Function.x, Function.y) %>%
  dplyr::filter(Function.x %in% c("V138", "V209", "V162", "V8") | Function.y %in% c("V138", "V209", "V162", "V8")) %>% View()
...
plot_biomarkers <- function(index, trim_length = 55) {
  ...
  biom_df <- fn_models %>%
    dplyr::filter(gene == paste("V", index, sep = "")) %>%
    dplyr::mutate(Feature_Name = str_sub(Feature_Name, 1, trim_length)) %>%
    dplyr::mutate(Feature_Name = tidytext::reorder_within(Feature_Name, importance, model))
